...
program
  .version('0.0.1')
  .description(
    `TS client that connects to a TS server at the specified ws:// URL and to a Wall Clock server at the specified udp://<host>:<port> URL. It then requests a timeline using the specified contentIdStem and timelineSelector and assumes the timeline is running at the specified tickrate.`,
  )
...
udpSocket.on('listening', function () {
  ...
  var _wcClient = createWcClient(udpSocket, wallClock, protocolOptions);
});
...
ws.on('open', function () {
  ...
  var _client = createTsClient(ws, timelineClock, options);

  setInterval(function () {
    console.log(
      'Sync timeline position = ',
      timelineClock.now().toFixed(5),
      '   speed = ',
      timelineClock.speed.toFixed(2),
      '    Availability = ',
      timelineClock.isAvailable(),
    );
  }, 1000);
});
