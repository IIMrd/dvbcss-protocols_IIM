...
program
  .version('0.0.1')
  .description(
    `WebSocket Wall clock server bound to the specified interface (default ${BIND_HOST}) and port (default ${BIND_PORT}) at a given resource path (default ${RES_PATH}).`,
  )
...
var _precision = sysClock.dispersionAtTime(sysClock.now());
var protocolOptions = {
...
app.websocket(RES_PATH, function (info, cb, _next) {
  cb(function (ws) {
    var _wcServer = createServer(ws, wallClock, protocolOptions);
