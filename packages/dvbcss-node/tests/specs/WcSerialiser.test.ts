...
describe('JsonSerialiser', () => {
  ...
  it('serialises and deserialises correctly a simple message', () => {
    const msg = new WallClockMessage(
      0,
      WallClockMessageTypes.request,
      1,
      256.7,
      2000,
      145245,
    ...
    expect(JsonSerialiser.pack(msg)).toEqual(
      JSON.stringify({
        v: 0,
        t: 0,
        p: 1,
  ...
  it('serialises and deserialises correctly a different message', () => {
    const msg = new WallClockMessage(
      0,
      WallClockMessageTypes.responseWithFollowUp,
      0.000001,
      32.51,
      3993932000,
      145245,
...
describe('BinarySerialiser', () => {
  ...
  it('serialises and deserialises correctly even with an unusual originate_timevalue', () => {
    const msg = new WallClockMessage(
      0,
      WallClockMessageTypes.response,
      8,
      500,
      0xaabbccdd,
      0xeeff1122,
    ...
    const expectedPackedMsg = makeBinaryMsg([
      0x00, 0x01, 0x03, 0x00, 0x00, 0x01, 0xf4, 0x00, 0xaa, 0xbb, 0xcc, 0xdd, 0xee, 0xff, 0x11,
      0x22, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x07, 0xd0, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
      0x0b, 0xb8,
    ]);
