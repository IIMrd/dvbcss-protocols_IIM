...
import { CorrelatedClock, DateNowClock } from '@iimrd/dvbcss-clocks';
...
class MockWebSocket extends EventEmitter {
  ...
  public dispatchEvent(event: Event): boolean {
    return (this as any).emit(event.type, event);
  }
}
...
describe('TSClientProtocol', () => {
  ...
  const options = {
    ...
    contentIdStem: 'http://myserver.org/myvideo.mp4',
    tickrate: 2000,
  };
  ...
  it('sends setup message when start is called from WebSocketAdaptor.', () => {
    const tscp = new TSClientProtocol(clock, options);
    new WebSocketAdaptor(tscp, ws as any);

    ws.triggerEvent('open', {});
    expect(ws.send).toHaveBeenCalledWith(
      new TSSetupMessage(options.contentIdStem, options.timelineSelector).serialise(),
      {
        binary: false,
      },
    );
