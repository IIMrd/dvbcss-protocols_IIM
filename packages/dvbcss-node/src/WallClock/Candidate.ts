...
export class Candidate {
  ...
  constructor(wcMsg: WallClockMessage, nanosRx: number) {
    ...
    this.t1 = WallClockMessage.secsAndNanosToNanos(
      wcMsg.originate_timevalue_secs,
      wcMsg.originate_timevalue_nanos,
    );
  ...
  public toCorrelation(clock: CorrelatedClock): Correlation {
    ...
    const c = new Correlation({
      ...
      childTime: (t2 + t3) / 2,
      initialError:
        (this.precision + rtt / 2 + mfeC * (this.t4 - this.t1) + mfeS * (this.t3 - this.t2)) /
