...
export class WallClockClientProtocol extends EventEmitter implements ProtocolHandler {
  ...
  private dest: any;
  private responseCache: Map<string, ReturnType<typeof setTimeout>> = new Map();
  ...
  constructor(
    ...
    options: WallClockClientProtocolOptions = {},
  ) {
    super();
    ...
    this.wallClock.correlation = this.wallClock.correlation.butWith({
      initialError: Number.POSITIVE_INFINITY,
    });
    this.wallClock.speed = 1;
    this.wallClock.availabilityFlag = false;

    this.altClock = new CorrelatedClock(this.parentClock, {
      tickRate: wallClock.tickRate,
      correlation: wallClock.correlation,
    });
  ...
  private sendRequest(): void {
    ...
    let msg: WallClockMessage | string | ArrayBuffer | Uint8Array = WallClockMessage.makeRequest(
      t[0],
      t[1],
    );
   ...
   */
  public handleMessage(msg: string | ArrayBuffer, _routing: any): void {
