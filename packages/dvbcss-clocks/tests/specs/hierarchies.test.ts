...
describe('For hierarchies of clocks', function () {
  it('when toOtherClockTime() is called, an error is thrown if the two clocks are not part of the same hierarchy (and therefore have no common ancestor)', function () {
    ...
    expect(function () {
      a.toOtherClockTime(b, 5);
    }).toThrow();
    expect(function () {
      a.toOtherClockTime(b1, 5);
    }).toThrow();
    expect(function () {
      a.toOtherClockTime(b2, 5);
    }).toThrow();

    expect(function () {
      a1.toOtherClockTime(b, 5);
    }).toThrow();
    expect(function () {
      a1.toOtherClockTime(b1, 5);
    }).toThrow();
    expect(function () {
      a1.toOtherClockTime(b2, 5);
    }).toThrow();

    expect(function () {
      a2.toOtherClockTime(b, 5);
    }).toThrow();
    expect(function () {
      a2.toOtherClockTime(b1, 5);
    }).toThrow();
    expect(function () {
      a2.toOtherClockTime(b2, 5);
    }).toThrow();

    expect(function () {
      b.toOtherClockTime(a, 5);
    }).toThrow();
    expect(function () {
      b.toOtherClockTime(a1, 5);
    }).toThrow();
    expect(function () {
      b.toOtherClockTime(a2, 5);
    }).toThrow();

    expect(function () {
      b1.toOtherClockTime(a, 5);
    }).toThrow();
    expect(function () {
      b1.toOtherClockTime(a1, 5);
    }).toThrow();
    expect(function () {
      b1.toOtherClockTime(a2, 5);
    }).toThrow();

    expect(function () {
      b2.toOtherClockTime(a, 5);
    }).toThrow();
    expect(function () {
      b2.toOtherClockTime(a1, 5);
    }).toThrow();
    expect(function () {
      b2.toOtherClockTime(a2, 5);
    }).toThrow();
  });
  ...
  it("toOtherClockTime() can be used to convert to a parent's parent's time", function () {
    ...
    expect(a2.toOtherClockTime(a, 500)).toEqual(a1.toParentTime(a2.toParentTime(500)));
  });
  ...
  it('toOtherClockTime() can be used to convert between ancestors in a linear hierarchy', function () {
    ...
    var _a4 = new CorrelatedClock(a3, { tickRate: 28, correlation: new Correlation(17, 9) });

    expect(a3.toOtherClockTime(a1, 500)).toEqual(a2.toParentTime(a3.toParentTime(500)));
  });
